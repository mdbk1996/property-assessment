  ...
  return (
    <div style={{ marginBottom: '28px' }}>
      ...
      <div style={{
        ...
        <div style={{ fontSize: '11px', color: '#6b5c47', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '10px' }}>Investment Verdict</div>
        <div style={{ fontSize: '22px', fontFamily: "'Cormorant Garamond', serif", fontWeight: 700, color, lineHeight: 1.3 }}>{rec}</div>
        <div style={{ marginTop: '14px', display: 'flex', justifyContent: 'center', gap: '32px', fontSize: '13px', color: '#6b5c47' }}>
          <span>Flip Profit: <span style={{ color: flipProfit > 0 ? '#5cba7d' : '#e05c5c' }}>{formatCurrency(flipProfit)}</span></span>
          <span>Hold Cash Flow: <span style={{ color: holdCashFlow > 0 ? '#5cba7d' : '#e05c5c' }}>{formatCurrency(holdCashFlow)}/mo</span></span>
          <span>CoC: <span style={{ color: holdCoCReturn > 5 ? '#5cba7d' : '#e05c5c' }}>{formatPct(holdCoCReturn)}</span></span>
        </div>
      </div>
    </div>
  );
